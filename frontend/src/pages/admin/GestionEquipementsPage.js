...
// CRUD complet + sous-équipements + QR code + impression
// ============================================================
import React, { useState, useEffect, useCallback, useRef } from 'react';
import { equipementsAPI, sousEquipAPI } from '../../api';
import QrInterventionModal from '../../components/QrInterventionModal';
import { validerNom, validerFormulaire } from '../../utils/validators';

const FAMILLES = [
  { value: '', label: '— Sélectionner —' },
  { value: 'ROOFTOP', label: 'ROOFTOP' },
  { value: 'Compresseurs', label: 'Compresseurs' },
  { value: 'Manutention-Gerbeur', label: 'Manutention — Gerbeur' },
  { value: 'Manutention-MontéCharge', label: 'Manutention — Monté charge' },
];
...
const STATUT_LABEL = { actif: 'Actif', en_panne: 'En panne', en_maintenance: 'En maintenance', hors_service: 'Hors service' };

const FieldError = ({ erreur }) => erreur ? <p className="text-red-600 text-xs mt-1">{erreur}</p> : null;

export default function GestionEquipementsPage() {
  ...
  const [eqErreurs, setEqErreurs]     = useState({});
  ...
  // ── CRUD Équipements ──────────────────────────────────────
  const validerEqForm = () => {
    const { valide, erreurs } = validerFormulaire({
      nom: validerNom(eqForm.nom, "Le nom de l'équipement"),
    });
    setEqErreurs(erreurs);
    return valide;
  };

  const handleEqSubmit = async (e) => {
    e.preventDefault();
    if (!validerEqForm()) return;
    setErreur('');
    try {
      if (editingEq) {
  ...
  // ── CRUD Sous-équipements ─────────────────────────────────
  const handleSeSubmit = async (e) => {
    e.preventDefault();
    setSeErreur('');
    if (!seForm.nom.trim()) { setSeErreur('Le nom du sous-équipement est obligatoire.'); return; }
    try {
      if (editingSe) {
  ...
  const equipementsFiltres = equipements.filter((e) =>
    ...
    (e.famille_equipement || '').toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-full">
      ...
      <div className="flex flex-wrap justify-between items-center gap-4 mb-6">
        ...
        <button
          onClick={() => { setEditingEq(null); setEqForm({ nom: '', famille_equipement: '' }); setEqErreurs({}); setErreur(''); setShowEqModal(true); }}
            ...
            <table className="min-w-full">
              <thead>
                <tr className="bg-gradient-to-r from-blue-900 to-blue-800">
                  ...
                  <th className="px-5 py-3 text-left text-xs font-semibold text-blue-100 uppercase tracking-wider">Famille</th>
                ...
                {equipementsFiltres.map((eq, idx) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5 text-sm text-gray-500">
                      {eq.famille_equipement
                        ? <span className="px-2 py-0.5 bg-blue-50 text-blue-700 rounded-full text-xs font-medium">{eq.famille_equipement}</span>
                      ...
                      <div className="flex items-center justify-end gap-2">
                        <button
                          ...
                          className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium transition ${
                            panelEq?.id === eq.id ? 'bg-blue-600 text-white' : 'bg-blue-50 text-blue-700 hover:bg-blue-100'
                          }`}
                        >
                          🔩 Sous-équip.
                        </button>
                        ...
                        <button
                          onClick={() => { setEditingEq(eq); setEqForm({ nom: eq.nom, famille_equipement: eq.famille_equipement || '' }); setEqErreurs({}); setErreur(''); setShowEqModal(true); }}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
        {[
          ...
          { icon: '📊', label: 'Familles distinctes', value: new Set(equipements.map(e => e.famille_equipement).filter(Boolean)).size, color: 'bg-purple-50 text-purple-900' },
        ].map((s) => (
      ...
      {showEqModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            ...
            <form onSubmit={handleEqSubmit} className="p-6 space-y-4" noValidate>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">Nom de l'équipement *</label>
                <input
                  type="text"
                  value={eqForm.nom}
                  onChange={(e) => { setEqForm({ ...eqForm, nom: e.target.value }); setEqErreurs(p => ({...p, nom: null})); }}
                  className={`w-full px-4 py-2.5 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm ${eqErreurs.nom ? 'border-red-400' : 'border-gray-200'}`}
                  placeholder="Ex : Groupe électrogène, Pompe..."
                />
                <FieldError erreur={eqErreurs.nom} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">Famille d'équipement</label>
                <select
                  value={eqForm.famille_equipement}
                  onChange={(e) => setEqForm({ ...eqForm, famille_equipement: e.target.value })}
