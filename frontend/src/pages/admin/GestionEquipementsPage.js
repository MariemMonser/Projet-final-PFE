    ...
    <div className="p-6 bg-gray-50 min-h-full">
      ...
      <div className="flex flex-wrap justify-between items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">Mode consultation — sélectionnez un équipement pour voir ses sous-équipements</p>
        </div>
        <span className="flex items-center gap-2 px-4 py-2 bg-amber-100 text-amber-800 rounded-lg text-sm font-medium border border-amber-200">
          👁️ Mode consultation
        </span>
      </div>
              ...
              {seLoading ? (
                ...
                <ul className="space-y-2.5">
                  {sousEquips.map((se) => {
                    ...
                    return (
                      <li key={se.id} className="flex items-center justify-between p-3.5 bg-gray-50 rounded-xl border border-gray-100">
                        ...
                        <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ml-3 flex-shrink-0 ${cfg.badge}`}>
                          {cfg.icon} {cfg.label}
                        </span>
                      </li>
                    );
